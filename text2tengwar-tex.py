# Pathched version from https://github.com/bernardinelli/py-tengwar-tex/blob/master/transcriber.py
...
Usage: the load_and_transcribe function gets a text in Roman
alphabet and transcribe for the Tengwar LaTeX codes.
...
def tengwar_roman_to_tex(text):
    return transcriber(replacer(text))


def load_and_transcribe(file_name):
    scribe = ''
...
def main():
    ...
    parser.add_argument('-i', '--input_file', type=str, help="Input text file path.")
    parser.add_argument('-o', '--output_file', type=str, help="Output TeX tengwar file path.")
    args = parser.parse_args()

    if args.input_file is None:
        content = sys.stdin.read()
        tengwar = '\n'.join(tengwar_roman_to_tex(l) for l in content.splitlines())
    else:
        tengwar = load_and_transcribe(args.input_file)
